...
## 🆕 최근 세션 (2026-08-01~03) — 접수폼 라이브 + 페스티벌 콘텐츠 확정

### ✅ 라이브 배포 완료

**접수폼 `/apply/competition`** — `feat/comp-entry` → master 머지 완료
- 복수 종목 체크박스 + 참가비 합산, 선수사진 업로드, **국적 선택(227개국, `src/lib/countries.ts`)**
- 상단 가로 타이틀 배너(`apply_title_2026.jpg` 1600x500, v2 — 주최·주관 표기 정상)
- 접수 전 확인 박스 (`CompetitionBrief.tsx`) — 선착순 아님·복수 신청·비기너 자격·입금 후 확정
- 비기너 참가 자격: 2023-01-01 이후 입문 + **국내외 서핑대회 입상자 제외** (체크박스 문구에도 포함)
- ⚠️ 8/5 09시 전까지 "접수 중인 대회가 없습니다" 표시 = 정상 (운영 플래그 OFF)

**`/festival`**
- 공식 배너 PC(2560x1440)/모바일(1080x1620) art direction, v2 문구 수정본
- 대회 3종: 비기너 서핑대회 · 코리아 오픈 SUP 레이싱 · 코리아 오픈 숏보드·롱보드·SUP 서핑
- 시상 추가 (비기너·SUP 레이싱 1위 30만/2위 20만/3위 10만, 웨이브 3종은 8/6 공지 시)
- 주최·주관·후원 섹션, 장소(기사문·설악·물치), **"웨이브" 용어 전면 제거**
- 주요 일정 세부 타임라인은 비공개로 제거 (제목·설명만)

**전역**: `word-break: keep-all` (한글 단어 중간 줄바꿈 해결) · `/festival`·`/apply/competition` 전용 OG 이미지(`og-festival.jpg`, 두 페이지 공용)

### 📌 확정 사항 (2026-08-03, 신청서 v02 와 상충하던 것)
| 항목 | 확정 | 신청서 v02(구버전) |
|---|---|---|
| 비기너 보드 | 전원 동일 스펀지보드 | 9피트 이상 개인 롱보드 |
| 종목 중복 신청 | 가능 (참가비 합산) | 불허 |
| 부문별 인원 | 제한 없음 | 10인 이상 시 개최 |
| 접수 시작 | **8/5 09:00** (8/4→변경) | — |

공식 명칭: 페스티벌 「2026 양양서핑페스티벌」 / 대회 「대한서핑협회장배 서핑대회」 (주최 양양군, 주관 YSA·KSA)

### 🚨 LINEUP 채팅 대기 (8/5 09시 접수 오픈 전 필수)
브리프: `cert-manager\docs\2026-entry-api-brief.md`
1. 비기너 부문명 "비기너 - 롱보드 남자/여자" → **"남자부"/"여자부"** (스테이징·운영 rename)
2. 운영 `NEXT_PUBLIC_COMPETITION_ENABLED=1` + 2026 대회 등록 (현재 comp-entry 404)
3. 접수 알림톡 미구현 — 발송 시점·문안 확정 + 카카오 템플릿 심사

### 📝 인스타
- 8/1 포스터 공지 완료 / 8/3 비기너·SUP 모집요강 공지 **완료**
- 캡션 원본: 세션 스크래치패드 (긴 버전 1545자 / 압축 1121자)
- ⚠️ **포스터 이미지의 주최·주관이 뒤바뀌어 인쇄됨** (배너는 정상) — 재사용 전 수정 필요
- 다음: 8/6 코리아 오픈 숏보드·롱보드·SUP 서핑 모집요강 (상금 포함, 접수 8/13)

### 🖥 작업 환경
gstack `browse` 사용 금지(Windows 콘솔 창 문제) → Claude 내장 `preview_start` 사용.
ysa-dev 포트는 **3013 고정 필수** (cert-manager CORS 화이트리스트).

---

## 🗂 이전 세션 (2026-07-29) — 2026 페스티벌·대회 허브 + 복수 종목 접수폼
