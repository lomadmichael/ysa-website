    ...
    <>
      <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-foam">
        <nav className="max-w-[1200px] mx-auto px-4 h-16 flex items-center justify-between" aria-label="메인 네비게이션">
          ...
          {/* Mobile Hamburger */}
          <button
            className="lg:hidden p-2 text-navy relative z-[60]"
      ...
      {/*
        Mobile Fullscreen Overlay — must render OUTSIDE <header>.
        Reason: header uses `backdrop-blur-sm` (backdrop-filter), which creates
        a CSS containing block for descendants with position:fixed. A nested
        `fixed inset-0` then positions relative to the 64px-tall header, giving
        the overlay height: 0. Rendering as a sibling escapes that context.
      */}
      {mobileOpen && (
