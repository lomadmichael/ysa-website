 ...
 *   token = "<만료 epoch(초)>.<HMAC-SHA256(만료, key=SURFCAMP_ADMIN_PASSWORD)>"
...
export const ADMIN_COOKIE = 'surfcamp_admin';
/** 쿠키를 관리자 경로에만 붙인다. 접수 폼 요청에는 세션 쿠키가 실려 가지 않는다. */
export const ADMIN_COOKIE_PATH = '/apply/surf-camp/admin';
export const ADMIN_TTL = TTL;

/**
 * 운영 배포에 비밀번호가 없으면 관리자 화면이 "비밀번호 없이 열리는" 상태가 되므로
 * 모듈 로드 시점에 즉시 실패시킨다. 로컬/프리뷰(NODE_ENV !== 'production')는 경고만.
 */
if (!process.env.SURFCAMP_ADMIN_PASSWORD && process.env.NODE_ENV === 'production') {
  throw new Error('SURFCAMP_ADMIN_PASSWORD 미설정 — 서핑캠프 관리자 화면을 열 수 없습니다.');
}
