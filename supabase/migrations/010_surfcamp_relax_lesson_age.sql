-- ============================================================================
-- 2026 양양 서핑캠프 — 서핑강습 참가자격 완화 (009 의 변경분만)
--
-- 배경
--   원안은 "만 11세 이상 & 신장 130cm 이상"이었으나, 서핑스쿨에 따라 저연령
--   강습이 가능한 곳이 있어 정책을 바꿨다.
--     * 하드 게이트는 "만 9세" 하나뿐. 만 9세 미만은 서핑강습 신청 불가.
--     * 신장 제한은 삭제. 신장은 장비 준비·배정 참고용으로만 수집한다.
--     * 만 11세 미만 또는 신장 130cm 미만은 "저연령 배정" 대상이지만
--       접수 자체는 막지 않는다(앱 계층에서 안내 + 관리자 명단 표시).
--
-- ★ 이 파일은 자격 검사 조건만 바꾼다. 정원 판정 · 대기 승급 · 중복 검사 ·
--   batch 처리 등 나머지 본문은 009 와 100% 동일하게 유지한다.
...
-- 서핑강습 자격(만 9세 이상) 하드 백스톱.
-- 신장 검사는 제거했다 — 어떤 경로로 INSERT 해도 나이 하한만 막는다.
...
BEGIN
  ...
  IF v_age < 9 THEN
    RAISE EXCEPTION 'surfcamp_lesson_ineligible: % (age=%, min_age=9)', v_name, v_age
      USING ERRCODE = 'check_violation';
  END IF;
  RETURN NEW;
END $$;

-- 참가자 나이를 낮춰 자격을 깨는 수정도 막는다(신장은 더 이상 자격 요소가 아니다).
...
-- ============================================================================
-- 2. RPC 재정의 (009 본문 그대로 + 자격 검사 조건만 변경)
-- ============================================================================
...
BEGIN
  ...
  FOR v_i IN 0 .. v_n - 1 LOOP
    ...
    IF 'lesson' = ANY(v_progs) THEN
      -- 서핑강습 하드 게이트: 만 9세 이상 (신장은 배정 참고용이라 막지 않는다)
      IF v_age < 9 THEN
...
-- ── 2.2 수정 ─────────────────────────────────────────────────────────────────
-- 정원 재판정 규칙(R1~R5)은 009 와 동일하다. 자격 검사 조건만 나이 하한으로 바뀌었다.
...
BEGIN
  ...
  FOR v_i IN 0 .. v_n - 1 LOOP
    ...
    IF NOT (v_progs <@ ARRAY['lesson','special']::text[]) THEN
      RETURN jsonb_build_object('ok', false, 'error', 'invalid_program'); END IF;
    -- 서핑강습 하드 게이트: 만 9세 이상 (신장은 배정 참고용이라 막지 않는다)
    IF 'lesson' = ANY(v_progs) AND v_age < 9 THEN
      RETURN jsonb_build_object('ok', false, 'error', 'ineligible_lesson',
                                'name', v_p->>'name', 'age', v_age, 'height_cm', v_h);
    END IF;
  END LOOP;
